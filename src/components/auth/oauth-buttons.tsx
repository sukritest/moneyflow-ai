...
function GoogleIcon() {
  return (
    <svg viewBox="0 0 24 24" className="h-4 w-4" aria-hidden="true">
      ...
      <path fill="#FBBC05" d="M5.84 14.14a6.6 6.6 0 0 1 0-4.28V7.02H2.18a11 11 0 0 0 0 9.96l3.66-2.84z"/>
      <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15A10.97 10.97 0 0 0 12 1a10.99 10.99 0 0 0-9.82 6.02l3.66 2.84c.87-2.6 3.3-4.48 6.16-4.48z"/>
    </svg>
  );
}
...
export function OAuthButtons() {
  ...
  async function handleGoogleSignIn() {
    if (!SUPABASE_CONFIGURED) { setError("Supabase isn't configured yet."); return; }
    setError(null);
    setLoading("google");
    try {
      ...
      if (!popup) { setError("Popup was blocked. Please allow popups for this site and try again."); setLoading(null); return; }
      const handler = async (event: MessageEvent) => {
        if (event.origin !== window.location.origin) return;
        if (event.data?.type === "google-oauth-error") { setError("Google sign-in failed: " + event.data.error); setLoading(null); return; }
        if (event.data?.type === "google-id-token") {
          window.removeEventListener("message", handler);
          const { error: signInError } = await supabase.auth.signInWithIdToken({ provider: "google", token: event.data.idToken, nonce: rawNonce });
          if (signInError) { setError(signInError.message); setLoading(null); }
          else { router.push("/dashboard"); }
        }
      };
      window.addEventListener("message", handler);
      const checkClosed = setInterval(() => {
        if (popup.closed) { clearInterval(checkClosed); window.removeEventListener("message", handler); setLoading(null); }
      }, 500);
    } catch { setError("Failed to start Google sign-in. Please try again."); setLoading(null); }
  }
  ...
  async function handleLineOAuth() {
    ...
    const { error: oauthError } = await supabase.auth.signInWithOAuth({ provider: "line" as any, options: { redirectTo: window.location.origin + "/auth/callback" } });
  ...
  return (
    <div className="space-y-2">
      <div className="grid grid-cols-2 gap-2">
        <Button type="button" variant="outline" disabled={loading !== null} onClick={handleGoogleSignIn}>
          <GoogleIcon />
          <span className="ml-2">{loading === "google" ? "..." : "Google"}</span>
        </Button>
        <Button type="button" variant="outline" disabled={loading !== null} onClick={handleLineOAuth}>
          <LineIcon />
          <span className="ml-2">{loading === "line" ? "..." : "LINE"}</span>
        </Button>
      </div>
      {error && <p className="text-xs text-destructive">{error}</p>}
      <p className="sr-only">{t("auth.continueWithGoogle")} / {t("auth.continueWithLine")}</p>
    </div>
  );
}
